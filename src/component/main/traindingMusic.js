...
export default function TraindingMusic({ music, path, AddToCart }) {
    return (
        <div>
            <p className={style.bookTitle}>{path === process.env.REACT_APP_API_BASEURL ? 'More in this series' : 'Trending in Music and Video'} </p>
                ...
                {music.map((book, index) => {
                    return (
                        <div className={style.bookCard} key={index}>
                            ...
                            {/* <img src={`${book.img}`} alt='Trending book' /> */}
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 13 12" className={style.whishlistSvg}>
                            ...
                            <p className={style.price}>LBP {book.price}</p>
